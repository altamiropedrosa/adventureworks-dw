with
    ...
    --    select sk_cartao_credito, id_cartao_credito, cd_tipo_cartao_credito, nm_responsavel_cartao_credito
    --    from {{ ref('dim_cartoes_credito') }}
    --)
    ,dim_clientes as (
        select sk_cliente, id_cliente, nm_cliente, nm_cidade, nm_estado, nm_pais, ds_grupo_territorio
    ...
    ,stg_vendas_pedidos_motivos as (
        select id_pedido_venda, max(id_motivo_venda) as id_motivo_venda
        from {{ ref('stg_sap_adw_vendas_pedidos_motivos') }}
        group by id_pedido_venda
    )
    ,dim_motivos_vendas as (
        select sk_motivo_venda, id_motivo_venda, nm_motivo_venda, cd_motivo_venda
    ...
    ,dim_promocoes as (
        select sk_promocao, id_promocao, nm_promocao, ds_tipo_promocao, ds_categoria_promocao
    ...
    ,dim_vendedores as (
        select sk_vendedor, id_vendedor, nm_vendedor, cd_pais_atuacao, nm_pais_atuacao, ds_grupo_territorio_atuacao
    ...
    ,join_tables as (
        select 
            ...
            ,cli.nm_estado
            ,cli.nm_pais
            ,cli.ds_grupo_territorio
            --DADOS DO VENDEDOR
            ,ven.sk_vendedor
            ,ven.nm_vendedor
            ,ven.cd_pais_atuacao
            ,ven.nm_pais_atuacao
            ,ven.ds_grupo_territorio_atuacao
            --DADOS DE ENTREGA
            ,ter.sk_territorio
            ...
            --,car.nm_responsavel_cartao_credito
        ...
        left join stg_vendas_pedidos_motivos venmot on venmot.id_pedido_venda = pedite.id_pedido_venda
        left join dim_motivos_vendas mot on mot.id_motivo_venda = venmot.id_motivo_venda
